...
封面建议：**“做了个 AI 之星鉴定器：专治听完很燃，回去不会干”**。

30 秒演示结构：先展示自写的虚构高分文本 → 出示三处原文证据 → 切到 `/translate` → 用 `/challenge` 追问“到底替代哪一步、怎么验收？” → 展示具体版本的低分结果。屏幕始终保留“评价文本，不评价人”。

可直接改写的发布文案：

> 我把 AI 圈常见话术做成了一个开源 Skill：AI 之星鉴定器。它不鉴定谁懂不懂 AI，只拆一段话里哪些是术语、哪些是比喻、哪些能验证。支持鉴定、翻译、追问和讽刺生成。最有用的可能不是分数，而是它最后问的那句：你到底想让 AI 替代哪一步？示例为虚构，分数只表示文本话术特征。

发布截图用 [分数卡模板](assets/ai-star-scorecard.md)；移除姓名、头像、账号与项目敏感信息。保留原文上下文、模式和版本说明，不把分数改成“某某是骗子”的结论。生成模式的虚构标记不要裁掉。
